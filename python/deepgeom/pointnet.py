...
class PointNetAttentionPool(nn.Module):
    def __init__(self, MLP_dims, Attention_dims, FC_dims, MLP_doLastRelu=False):
        ...
        self.S = nn.Sequential(PointwiseMLP(Attention_dims, doLastRelu=False), nn.Softmax(dim=-2))  # BxNxM  # BxNxM
...
class PointNetBilinearPool(nn.Module):
    def __init__(self, MLP1_dims, FC1_dims, MLP2_dims, FC2_dims, FC_dims):
        ...
        self.F1 = nn.Sequential(PointNetGlobalMax(MLP1_dims), *get_MLP_layers(FC1_dims, False))  # BxFC1_dims[-1]
        self.F2 = nn.Sequential(PointNetGlobalMax(MLP2_dims), *get_MLP_layers(FC2_dims, False))  # BxFC2_dims[-1]
        self.L = nn.Sequential(*get_MLP_layers(FC_dims, False))
...
class BoostedPointPairNet(PointPairNet):
    def __init__(self, d, dims, FC_dims, max_pool=True):
        ...
        self.d = d
        self.add_module("BoostPool", nn.AdaptiveMaxPool1d(1) if max_pool else nn.AdaptiveAvgPool1d(1))
...
class BoostedPointPairNet2(nn.Module):
    ...
    def __init__(self, boost_factor, dims, FC_dims, sym_pool_max=True, boost_pool_max=True):
        ...
        self.L = nn.Sequential(*get_MLP_layers(dims, False))
        self.SymPool = (
            nn.AdaptiveMaxPool3d((1, 1, dims[-1])) if sym_pool_max else nn.AdaptiveAvgPool3d((1, 1, dims[-1]))
        )
        self.F = nn.Sequential(*get_MLP_layers(FC_dims, False))
        self.BoostPool = (
            nn.AdaptiveMaxPool2d((1, FC_dims[-1])) if boost_pool_max else nn.AdaptiveAvgPool2d((1, FC_dims[-1]))
        )
...
class PointNetAllLayers(nn.Sequential):
    def __init__(self, MLP_dims, MLP_doLastRelu=False):
        layers = []

        for i in range(len(MLP_dims) - 1):
            one_layer = PointwiseMLP((MLP_dims[i], MLP_dims[i + 1]), doLastRelu=MLP_doLastRelu)
            layers.append(one_layer)

        layers.append(GlobalPool(nn.AdaptiveMaxPool2d((1, MLP_dims[-1]))))

        super(PointNetAllLayers, self).__init__(*layers)
        self.layers = layers
...
class GlobalPoolAndFC(nn.Sequential):
    """BxNxdims[0] -> Bxdims[-1]"""

    def __init__(self, dims, FC_dims, doLastRelu=False):
        layers = [
            GlobalPool(nn.AdaptiveMaxPool2d((1, dims[-1]))),  # BxK
        ]

        layers.extend(get_MLP_layers(FC_dims, False))
        super(GlobalPoolAndFC, self).__init__(*layers)
